...
const Container = styled(Flex)`
  ...
  background: linear-gradient(0deg, rgba(39, 38, 44, 0.4), rgba(39, 38, 44, 0.4)),
    linear-gradient(180deg, #ffe66b 0%, #fed136 100%);
  ${({ theme }) => theme.mediaQueries.md} {
    padding: 0px;
    background: linear-gradient(180deg, #ffe66b 0%, #fed136 100%);
  }
`
...
const domain = 'https://beeswap.finance'
...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    ...
    return warningText.split(/(https:\/\/beeswap.finance)/g)
  }, [t])
    ...
    <Container className="warning-banner">
      {isMobile || isMd ? (
        ...
        <>
          <InnerContainer>
            <img
              src="/phishing-warning-bunny.png"
